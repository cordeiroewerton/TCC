module iir_filter #(
    ...
    logic signed [DATA_WIDTH-1:0] aux[5:0];
    logic signed [DATA_WIDTH-1:0] sum_1, sum_2, sum_3;
    logic signed [DATA_WIDTH-1:0] aux_sum1, aux_sum2;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            for (i = 0; i < (TAPS_A + TAPS_B) - 1; i++) aux[i] <= 1'b0;
            y_valid <= 1'b0;
        end else begin
            x_buf[0] <= x_in;
            x_buf[1] <= x_buf[0];
    ...
    add16se_2UY u0(.A(aux[0]), .B(aux[1]), .O(aux_sum1));
    add16se_2UY u2(.A(aux_sum1), .B(aux[2]), .O(acc_a));

    add16se_2UY u4(.A(aux[3]), .B(aux[4]), .O(aux_sum2));
    add16se_2UY u6(.A(aux_sum2), .B(aux[5]), .O(y_out));
endmodule
