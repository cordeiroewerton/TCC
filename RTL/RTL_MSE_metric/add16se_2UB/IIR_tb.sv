module iir_filter_tb();
	...
	logic signed [31:0] d_in, d_out;

	integer x_in, x_out; // Corrigido para 'integer'
	logic signed [31:0] x_read;
	...
	iir_filter u0(.clk(clk), .rst_n(rst_n), .x_in(d_in), .x_valid(1),.y_out(d_out), .y_valid(y_valid));
  	...
  	initial begin
		...
		rst_n <= 0;
		d_in <= 0;
		x_in = $fopen("/home/xmen-admin/Desktop/TCC/Sinais_de_entrada/signal_art.txt", "r"); // Corrigido para '='
		if (x_in == 0) begin
			$display("Erro ao abrir o arquivo 'valores.txt'");
			$finish;
		end
		x_out = $fopen("/home/xmen-admin/Desktop/TCC/Sinais_de_saida/saida_2UB.txt", "w"); // Corrigido para '='
		if (x_out == 0) begin
