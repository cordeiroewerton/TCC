module iir_filter #(
    ...
    add16se_2UB u0(.A(aux[0]), .B(aux[1]), .O(aux_sum1));
    add16se_2UB u2(.A(aux_sum1), .B(aux[2]), .O(acc_a));

    add16se_2UB u4(.A(aux[3]), .B(aux[4]), .O(aux_sum2));
    add16se_2UB u6(.A(aux_sum2), .B(aux[5]), .O(y_out));
endmodule
